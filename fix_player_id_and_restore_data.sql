-- ===================================================================
-- FIX PLAYER_ID + RESTAURAÇÃO COMPLETA DE DADOS
-- ===================================================================
-- PROBLEMA: Migration multi-tenant pode ter zerado player_id na tabela users
-- SOLUÇÃO: Restaurar dados E garantir player_id em ambas as tabelas
...
-- ===================================================================
TRUNCATE TABLE poker.session_confirmations CASCADE;
...
TRUNCATE TABLE poker.user_tenants CASCADE; -- Nova tabela multi-tenant
...
-- PARTE 2: RESTAURAR TENANT
-- ===================================================================
-- Estrutura real: id, name, email, phone, plan, status, max_users, max_sessions_per_month, created_at, updated_at, approved_at, suspended_at
...
VALUES (1, 'Casa Luis', 'luisfboff@hotmail.com', NULL, 'basic', 'active', 10, 50, '2025-10-19 21:26:02.895736+00', '2025-10-19 21:26:02.895736+00', NOW())
ON CONFLICT (id) DO NOTHING;

-- PARTE 3: RESTAURAR JOGADORES (13 jogadores)
-- ===================================================================
...
-- PARTE 4: RESTAURAR USUÁRIO (COM PLAYER_ID!)
-- ===================================================================
-- CRÍTICO: player_id = 1 vincula ao jogador "luis" para mostrar o gráfico
-- current_tenant_id = 1 para compatibilidade com migration multi-tenant
...
VALUES (
  ...
  NOW(),
  true,
  1, -- ⭐ PLAYER_ID = 1 (jogador "luis") - NECESSÁRIO PARA O GRÁFICO
  1, -- current_tenant_id (novo campo da migration multi-tenant)
  (SELECT id FROM auth.users WHERE email = 'luisfboff@hotmail.com' LIMIT 1)
)
ON CONFLICT (id) DO UPDATE SET
  player_id = 1, -- ⭐ FORÇAR player_id mesmo se já existir
  current_tenant_id = 1,
  supabase_user_id = (SELECT id FROM auth.users WHERE email = 'luisfboff@hotmail.com' LIMIT 1),
  updated_at = NOW();

-- PARTE 5: RESTAURAR USER_TENANTS (nova tabela multi-tenant)
-- ===================================================================
-- Esta tabela também precisa ter player_id configurado!
INSERT INTO poker.user_tenants (user_id, tenant_id, role, player_id, is_active, created_at, updated_at)
VALUES (
  1, -- user_id
  1, -- tenant_id
  'admin', -- role
  1, -- ⭐ PLAYER_ID = 1 aqui também (para compatibilidade futura)
  true,
  '2025-10-19 21:26:02.979882+00',
  NOW()
)
ON CONFLICT (user_id, tenant_id) DO UPDATE SET
  player_id = 1, -- ⭐ FORÇAR player_id
  role = 'admin',
  is_active = true,
  updated_at = NOW();

-- PARTE 6: RESTAURAR SESSÕES (2 sessões com dados completos)
-- ===================================================================

-- Sessão #4 (2025-10-21) - 9 jogadores, status=closed
INSERT INTO poker.sessions (
  ...
  4, 1, '2025-10-21', 'casa', 'closed', 1,
  '2025-10-22 12:30:17.877626+00', '2025-10-24 00:19:44.349708+00', NULL, NULL,
  ...
  0.00, 0.00, 0.00, 650.00, 800.00, 0.00, 9
) ON CONFLICT (id) DO NOTHING;

-- Sessão #5 (2025-09-17) - 9 jogadores, status=approved
INSERT INTO poker.sessions (
  id, tenant_id, date, location, status, created_by, created_at, updated_at, 
  approved_at, closed_at, players_data, recommendations, paid_transfers, 
  session_fee, janta_fee, rake_percentage, total_buyin, total_cashout, total_profit, players_count
) VALUES (
  5, 1, '2025-09-17', 'casa', 'approved', 1,
  '2025-10-24 00:18:53.498683+00', '2025-10-24 00:19:06.705777+00', NULL, NULL,
  ...
  0.00, 0.00, 0.00, 650.00, 650.00, 0.00, 9
) ON CONFLICT (id) DO NOTHING;

-- PARTE 7: ATUALIZAR SEQUENCES
-- ===================================================================
...
-- PARTE 8: VERIFICAÇÃO FINAL - DIAGNÓSTICO COMPLETO
-- ===================================================================

-- 8.1 Contagem de registros
SELECT '=== CONTAGEM DE REGISTROS ===' as verificacao;
SELECT 
  'TENANTS' as tabela, COUNT(*) as registros FROM poker.tenants
UNION ALL
SELECT 'USERS', COUNT(*) FROM poker.users
UNION ALL
SELECT 'USER_TENANTS', COUNT(*) FROM poker.user_tenants
UNION ALL
SELECT 'PLAYERS', COUNT(*) FROM poker.players
UNION ALL
SELECT 'SESSIONS', COUNT(*) FROM poker.sessions;

-- 8.2 ⭐ VERIFICAÇÃO CRÍTICA DO PLAYER_ID ⭐
SELECT '=== VERIFICAÇÃO PLAYER_ID (CRÍTICO PARA GRÁFICO) ===' as verificacao;
SELECT 
  u.id as user_id,
  u.name,
  u.email,
  u.player_id as "player_id (users table) ⭐",
  u.current_tenant_id,
  ut.player_id as "player_id (user_tenants table)",
  ut.role,
  p.id as actual_player_id,
  p.name as player_name,
  CASE 
    WHEN u.player_id IS NOT NULL AND u.player_id = 1 THEN '✅ OK - Gráfico vai aparecer'
    WHEN u.player_id IS NULL THEN '❌ ERRO - player_id NULL (gráfico NÃO aparece)'
    ELSE '⚠️ VERIFICAR - player_id diferente de 1'
  END as status_grafico
FROM poker.users u
LEFT JOIN poker.user_tenants ut ON ut.user_id = u.id AND ut.tenant_id = u.current_tenant_id
LEFT JOIN poker.players p ON p.id = u.player_id
WHERE u.id = 1;

-- 8.3 Detalhes das sessões
SELECT '=== SESSÕES RESTAURADAS ===' as verificacao;
SELECT 
  id,
  date,
  location,
...
-- 8.4 Verificar participação do jogador "luis" (id=1) nas sessões
SELECT '=== PARTICIPAÇÃO DO JOGADOR LUIS NAS SESSÕES ===' as verificacao;
SELECT 
  s.id as session_id,
  s.date,
  s.location,
  (
    SELECT jsonb_array_elements(s.players_data) 
    WHERE (jsonb_array_elements->>'id')::INTEGER = 1 
       OR (jsonb_array_elements->>'name') = 'luis'
    LIMIT 1
  ) as luis_player_data
FROM poker.sessions s
WHERE s.id IN (4, 5);

SELECT '=== ✅ RESTAURAÇÃO COMPLETA ===' as status;
